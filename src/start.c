...
void avisar_opcion_obsoleta(char *texto)
{
	...
	if (save_configuration_file_on_exit.v == 0)
		debug_printf(VERBOSE_ERR, "%s", texto);
}
	...
	while (!siguiente_parametro())
	{
		...
		*/
		...
		else if (!strcmp(argv[puntero_parametro], "--slotcard"))
		{
			...
			if (
				!util_compare_file_extension(argv[puntero_parametro], "epr") || !util_compare_file_extension(argv[puntero_parametro], "63") || !util_compare_file_extension(argv[puntero_parametro], "eprom") || !util_compare_file_extension(argv[puntero_parametro], "flash")
		...
		*/
		...
		else if (!strcmp(argv[puntero_parametro], "--enable-sdcard"))
		{
			command_line_sdcard.v = 1;
			debug_printf(VERBOSE_INFO, "SD Card enabled");
		}
		
		else if (!strcmp(argv[puntero_parametro], "--sdcard-type"))
		{
			siguiente_parametro_argumento();

			if (!strcmp(argv[puntero_parametro], "SDHC"))
			{
				sdcard.type = SDHC;
			}
			else if (!strcmp(argv[puntero_parametro], "SDXC"))
			{
				sdcard.type = SDXC;
			}
			else if (!strcmp(argv[puntero_parametro], "SDUC"))
			{
				sdcard.type = SDUC;
			}
			else
			...
			debug_printf(VERBOSE_INFO, "SD Card type: %d", sdcard.type);
		}

		else if (!strcmp(argv[puntero_parametro], "--sdcard-file"))
		{
			siguiente_parametro_argumento();
			...
			if (!si_ruta_absoluta(argv[puntero_parametro]))
			{
				...
				sprintf(sdcard.file_path, "%s/%s", directorio_actual, argv[puntero_parametro]);
			}
			else
			{
				sprintf(sdcard.file_path, "%s", argv[puntero_parametro]);
			}
			debug_printf(VERBOSE_INFO, "SD Card file: %s", sdcard.file_path);
		}
		...
		else if (!strcmp(argv[puntero_parametro], "--ay-stereo-channel"))
		{
			...
			if (canal == 'A')
				ay3_custom_stereo_A = valor;
			if (canal == 'B')
				ay3_custom_stereo_B = valor;
			if (canal == 'C')
				ay3_custom_stereo_C = valor;
		}
...
// Establecer rutas de utilidades externas
	...
	if (silence_detector_setting.v)
		debug_printf(VERBOSE_INFO, "Enabling Silence Detector");
	else
		debug_printf(VERBOSE_INFO, "Disabling Silence Detector");
	...
	if (command_line_zx8081_vsync_sound.v)
	...
	if (command_line_spectra.v)
		spectra_enable();

	if (command_line_ulaplus.v)
		enable_ulaplus();

	if (command_line_gigascreen.v)
		enable_gigascreen();

	if (command_line_16c.v)
		enable_16c_mode();

	if (command_line_interlaced.v)
		enable_interlace();

	if (command_line_timex_video.v)
		enable_timex_video();

	if (command_line_spritechip.v)
		spritechip_enable();

	if (command_line_chroma81.v)
		enable_chroma81();

	// SD Card
	if(command_line_sdcard.v)
	{
		sdcard_enable();
	}
	...
	if (command_line_zxmmc.v)
		zxmmc_emulation.v = 1;
	if (command_line_8bitide.v)
		eight_bit_simple_ide_enable();

	// IDE
	if (command_line_ide.v)
		ide_enable();
	...
	if (command_line_zxpand.v)
		zxpand_enable();

	// Dandanator
	if (command_line_dandanator.v)
		dandanator_enable();
	if (command_line_dandanator_push_button.v)
		dandanator_press_button();

	// Superupgrade
	if (command_line_superupgrade.v)
		superupgrade_enable(1);

	// Kartusho
	if (command_line_kartusho.v)
		kartusho_enable();

	// iFrom
	if (command_line_ifrom.v)
		ifrom_enable();
	...
	if (command_line_trd.v)
		trd_enable();

	if (command_line_dsk.v)
		dskplusthree_enable();
	...
	if (command_line_enable_midi.v)
	{
		if (audio_midi_output_init())
			debug_printf(VERBOSE_ERR, "Error initializing midi device");
	}
	...
	if (save_configuration_file_on_exit.v && do_no_show_changelog_when_update.v == 0)
	{
		...
		if (strcmp(last_version_string, BUILDNUMBER) && last_version_string[0] != 0)
		{ // Si son diferentes y last_version_string no es nula
