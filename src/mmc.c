...
int mmc_read_hdf_header(void)
{
	...
	if (mmc_file_header_hdf_pointer != NULL)
		free(mmc_file_header_hdf_pointer);
...
void mmc_cs(z80_byte value)
{
	...
	debug_printf(VERBOSE_INFO, "Card selected: %d", mmc_card_selected);
}
...
z80_byte mmc_read(void)
{
	...
	case 0x00:
		...
		if (MACHINE_IS_TBBLUE)
			return 0xFF; // Temporal. Sin esto no puede cargar el config.ini
		return 0;
		break;

	case 0x40:
		// Devuelve ok
		debug_printf(VERBOSE_INFO, "MMC Read command GO_IDLE_STATE");
		return 1;
		break;
	...
	*/
	...
	case 0x48:
		debug_printf(VERBOSE_INFO, "MMC Read command CMD8 SEND_IF_COND unhandled");
	...
	case 0x51:
		if (mmc_read_index >= 0)
		{
			...
			if (mmc_read_index >= 3 && mmc_read_index <= 514)
			{
				...
				debug_printf(VERBOSE_PARANOID, "MMC Read command READ_SINGLE_BLOCK. Index=%d PC=%d", mmc_read_index, reg_pc);
	...
	case 0x52:
		if (mmc_read_index >= 0)
		{
			...
			if (mmc_read_index >= 3 && mmc_read_index <= 514)
			{
				...
				debug_printf(VERBOSE_PARANOID, "MMC Read command READ_MULTIPLE_BLOCK. Index=%d PC=%d", mmc_read_index, reg_pc);
	...
	case 0x58:
		...
		if (mmc_write_index >= 0)
		{
			...
			// valor segundo, command response 0
			if (mmc_write_index == 1)
				value = 0;

			// valores siguientes
			if (mmc_write_index == 2)
				value = 0xff;
			if (mmc_write_index == 3)
				value = 0xff;

			// xxx0sss1
			// sss: status:
			// 010 data accepted
			if (mmc_write_index == 4)
				value = 4 + 1;

			if (mmc_write_index >= 5)
				value = 4 + 1;
...
void mmc_write(z80_byte value)
{
	...
	else
	{
		...
		// GO_IDLE_STATE
		case 0x40:
			debug_printf(VERBOSE_INFO, "MMC Write command GO_IDLE_STATE");
		...
		*/
		...
		// Parece que es de deteccion de MMC/SD
		case 0x48:
			debug_printf(VERBOSE_INFO, "MMC Write command CMD8 SEND_IF_COND unhandled");
			// mmc_r1 |=4; //devolver error
			break;
...
#define INICIO_WRITE_BLOCK_OFFSET 5
		...
		default:
			debug_printf(VERBOSE_INFO, "Received parameter for MMC unknown command 0x%02X", mmc_last_command);
			/*
