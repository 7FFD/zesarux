...
void set_visualmemreadbuffer(int dir)
{
	...
	if (valor < 255)
		visualmem_read_buffer[dir] = valor + 1;
...
void set_visualmemopcodebuffer(int dir)
{
	...
	if (valor < 255)
		visualmem_opcode_buffer[dir] = valor + 1;
}
...
void set_visualmemmmc_read_buffer(int dir)
{
	...
	if (valor < 255)
		visualmem_mmc_read_buffer[dir] = valor + 1;
}
...
void set_visualmemmmc_write_buffer(int dir)
{
	...
	if (valor < 255)
		visualmem_mmc_write_buffer[dir] = valor + 1;
}
...
void set_visualmemhilow_read_buffer(int dir)
{
	...
	if (valor < 255)
		visualmem_hilow_read_buffer[dir] = valor + 1;
}
...
void set_visualmemhilow_write_buffer(int dir)
{
	...
	if (valor < 255)
		visualmem_hilow_write_buffer[dir] = valor + 1;
}
...
// o So overflow flag = carry-out flag XOR carry from bit 6 into bit 7.
{
	...
	if ((result & 127) < (antes & 127))
		overflow67 = FLAG_C;
	else
		overflow67 = 0;

	if ((Z80_FLAGS & FLAG_C) ^ overflow67)
		Z80_FLAGS |= FLAG_PV;
	else
...
void cpu_common_jump_im01(void)
{
	...
	if (im_mode == 0 && z80_cpu_current_type == Z80_TYPE_MOSTEK)
		t_estados--;
}
...
z80_byte peek_byte_zx80_no_time(z80_int dir)
{
	...
	if (zxpand_enabled.v)
	{
		if (dir < 8192)
		{
			if (zxpand_overlay_rom.v)
				return zxpand_memory_pointer[dir];
		}
...
z80_byte peek_byte_no_time_spectrum_inves(z80_int dir)
{
	...
	if (dir < 16384)
	{
		if ((zesarux_zxi_registers_array[0] & 1) == 0)
		{
			...
			return memoria_spectrum[dir]; // Mostrar RAM baja
	}
...
z80_byte peek_byte_no_time_spectrum_16k(z80_int dir)
{
	...
	if (dir < 32768)
		...
		return idle_bus_port(255); // No hay memoria alli, devolvemos lo que la ula ha dejado en el bus
}
...
void poke_byte_no_time_zxuno(z80_int dir, z80_byte valor)
{
	...
	else
	{
		...
		if (dir > 16383 || (puerto_8189 & 1) == 1)
		{ // Si en ram, o rom con page RAM in ROM
#ifdef EMULATE_VISUALMEM
...
void poke_byte_no_time_msx1(z80_int dir, z80_byte valor)
{
	...
	if (tipo != MSX_SLOT_MEMORY_TYPE_RAM)
		return;
...
z80_byte peek_byte_no_time_msx1(z80_int dir)
{
	...
	if (tipo == MSX_SLOT_MEMORY_TYPE_EMPTY)
		return 255;
...
void poke_byte_no_time_svi(z80_int dir, z80_byte valor)
{
	...
	// Si esta vacio o es ROM, no hacer nada. O sea, si no es RAM
	if (tipo != SVI_SLOT_MEMORY_TYPE_RAM)
		return;
...
z80_byte peek_byte_no_time_svi(z80_int dir)
{
	...
	// Si esta vacio, retornar 0 o 255???
	if (tipo == SVI_SLOT_MEMORY_TYPE_EMPTY)
		return 255;
...
void poke_byte_no_time_coleco(z80_int dir, z80_byte valor)
{
	...
	// Si esta vacio o es ROM, no hacer nada. O sea, si no es RAM
	if (tipo != COLECO_SLOT_MEMORY_TYPE_RAM)
		return;
...
z80_byte peek_byte_no_time_coleco(z80_int dir)
{
	...
	// Si esta vacio, retornar 0 o 255???
	if (tipo == COLECO_SLOT_MEMORY_TYPE_EMPTY)
		return 255;
...
void poke_byte_no_time_sg1000(z80_int dir, z80_byte valor)
{
	...
	// Si esta vacio o es ROM, no hacer nada. O sea, si no es RAM
	if (tipo != SG1000_SLOT_MEMORY_TYPE_RAM)
		return;
...
z80_byte peek_byte_no_time_sg1000(z80_int dir)
{
	...
	// Si esta vacio, retornar 0 o 255???
	if (tipo == SG1000_SLOT_MEMORY_TYPE_EMPTY)
		return 255;
...
void poke_byte_no_time_sms(z80_int dir, z80_byte valor)
{
	...
	// Si esta vacio o es ROM, no hacer nada. O sea, si no es RAM
	if (tipo != SMS_SLOT_MEMORY_TYPE_RAM)
		return;
...
z80_byte peek_byte_no_time_sms(z80_int dir)
{
	...
	// Si esta vacio, retornar 0 o 255???
	if (tipo == SMS_SLOT_MEMORY_TYPE_EMPTY)
		return 255;
...
void poke_byte_no_time_chloe(z80_int dir, z80_byte valor)
{
	...
	if (dir < 16384)
	{
		...
		if (chloe_type_memory_paged[segmento] == CHLOE_MEMORY_TYPE_ROM)
			return;
	}
...
z80_byte *prism_return_segment_memory(z80_int dir)
{
	...
	if (puerto_8189 & 1)
	{
		...
		if (segmento == 6 || segmento == 7)
		{
			...
			if (page_type == 1)
				return prism_vram_mem_table[segmento - 4]; // VRAM 2,3
		}
	}
	...
	if (segmento == 2 || segmento == 3)
	{
		...
		if (prism_type_memory_paged[segmento] == PRISM_MEMORY_TYPE_HOME)
		{
			...
			else
			{
				...
				if (segmento == 3)
					return prism_ram_mem_table[ram_page_at_seg_2 + 1];
			}
		}
	}
...
void poke_byte_no_time_prism(z80_int dir, z80_byte valor)
{
	...
	if (dir < 16384)
	{
		// Segmento 0 o 1
		int segmento = dir / 8192;
		if (prism_type_memory_paged[segmento] == PRISM_MEMORY_TYPE_ROM)
			return;
	}
...
--   8. rom
	...
	if (dir < 16384)
	{
		if (tbblue_low_segment_writable.v == 0)
		{
			...
			if (!tbblue_is_writable_segment_mmu_rom_space(dir))
				return;
		}
	}
...
void poke_byte_no_time_chrome(z80_int dir, z80_byte valor)
{
	...
	if (dir < 16384)
	{
		if (!chrome_ram89_at_00())
			return; // No esta ram 8 o 9 en zona 0000-3fff. volver
	}
...
void poke_byte_no_time_timex(z80_int dir, z80_byte valor)
{
	...
	if (dir < 16384)
	{
		// Segmento 0 o 1
		int segmento = dir / 8192;
		if (timex_type_memory_paged[segmento] != TIMEX_MEMORY_TYPE_HOME)
			return;
	}
...
void poke_byte_no_time_sam(z80_int dir, z80_byte valor)
{
	...
	int segmento;
	segmento = dir / 16384;
	if (sam_memory_paged_type[segmento])
		return;
...
z80_byte lee_puerto_sam_no_time(z80_byte puerto_h, z80_byte puerto_l)
{
	...
	if (puerto_l == 249)
	{
		...
		if ((puerto_h & 1) == 0)
			valor = valor & puerto_teclado_sam_fef9;

		if ((puerto_h & 2) == 0)
			valor = valor & puerto_teclado_sam_fdf9;

		if ((puerto_h & 4) == 0)
			valor = valor & puerto_teclado_sam_fbf9;

		if ((puerto_h & 8) == 0)
			valor = valor & puerto_teclado_sam_f7f9;

		if ((puerto_h & 16) == 0)
			valor = valor & puerto_teclado_sam_eff9;

		if ((puerto_h & 32) == 0)
			valor = valor & puerto_teclado_sam_dff9;

		if ((puerto_h & 64) == 0)
			valor = valor & puerto_teclado_sam_bff9;

		if ((puerto_h & 128) == 0)
			valor = valor & puerto_teclado_sam_7ff9;
...
void out_port_pcw_no_time(z80_int puerto, z80_byte value)
{
	...
	*/
	...
	if (puerto_l == 0xAA && ay_chip_present.v)
		out_port_ay(65533, value);
	if (puerto_l == 0xAB && ay_chip_present.v)
...
void bit_bit_ixiy_desp_reg(z80_byte numerobit, z80_byte desp)
{
	...
	if (numerobit == 7 && (valor_leido & 128))
		Z80_FLAGS |= FLAG_S;
...
void bit_bit_cb_reg(z80_byte numerobit, z80_byte *registro)
{
	...
	else
	{
		...
		if (numerobit == 5 && (valor_leido & 32))
			Z80_FLAGS |= FLAG_5;
		else
			Z80_FLAGS &= (255 - FLAG_5);

		if (numerobit == 3 && (valor_leido & 8))
			Z80_FLAGS |= FLAG_3;
		else
			Z80_FLAGS &= (255 - FLAG_3);
	}
...
z80_byte lee_puerto_zx80_no_time(z80_byte puerto_h, z80_byte puerto_l)
{
	...
	if (puerto_l == 0x1d)
	{
		if (joystick_emulation == JOYSTICK_ZEBRA)
		{
			...
			if ((puerto_especial_joystick & 1))
				valor_joystick &= (255 - 8);
			if ((puerto_especial_joystick & 2))
				valor_joystick &= (255 - 4);
			if ((puerto_especial_joystick & 4))
				valor_joystick &= (255 - 2);
			if ((puerto_especial_joystick & 8))
				valor_joystick &= (255 - 1);
			if ((puerto_especial_joystick & 16))
				valor_joystick &= (255 - 16);
	...
	{
		if (puerto == ZESARUX_ZXI_ZX8081_PORT_REGISTER)
			return zesarux_zxi_read_last_register();
		if (puerto == ZESARUX_ZXI_ZX8081_PORT_DATA)
			return zesarux_zxi_read_register_value();
	}
...
#define SEQUENCE_ENTER2_MAX SEQUENCE_ENTER2_MIN + DURA_TECLA
	...
	if (initial_tap_sequence < SEQUENCE_ENTER2_MAX)
...
#define SEQUENCE2_ENTER2_MAX SEQUENCE2_ENTER2_MIN + DURA2_TECLA
	...
	if (initial_tap_sequence < SEQUENCE2_ENTER2_MAX)
		initial_tap_sequence++;
	else
		envia_jload_desactivar();
...
#define SEQUENCE3_ENTER2_MAX SEQUENCE3_ENTER2_MIN + DURA3_TECLA
	...
	if (initial_tap_sequence < SEQUENCE3_ENTER2_MAX)
		initial_tap_sequence++;
	else
		envia_jload_desactivar();
...
#define SEQUENCE_ZX80_ENTER2_MAX SEQUENCE_ZX80_ENTER2_MIN + DURA_ZX80_TECLA
	...
	if (initial_tap_sequence < SEQUENCE_ZX80_ENTER2_MAX)
		initial_tap_sequence++;
	else
		envia_jload_desactivar();
...
#define SEQUENCE_ZX81_ENTER2_MAX SEQUENCE_ZX81_ENTER2_MIN + DURA_ZX81_TECLA
	...
	if (initial_tap_sequence < SEQUENCE_ZX81_ENTER2_MAX)
		initial_tap_sequence++;
	else
		envia_jload_desactivar();
...
#define SEQUENCE_CPC_ENTER2_MAX SEQUENCE_CPC_ENTER2_MIN + DURA_CPC_TECLA
	...
	if (initial_tap_sequence < SEQUENCE_CPC_ENTER2_MAX)
		initial_tap_sequence++;
	else
	{
		envia_jload_desactivar();
...
#define SEQUENCE_SAM_ENTER2_MAX SEQUENCE_SAM_ENTER2_MIN + DURASAM_TECLA
	...
	if (initial_tap_sequence < SEQUENCE_SAM_ENTER2_MAX)
		initial_tap_sequence++;
	else
		envia_jload_desactivar();
...
z80_byte idle_bus_port(z80_int puerto)
{
	...
	if (MACHINE_IS_ZXUNO)
	{
		...
		if (devcontrol & 8)
		{ // A 1, esta deshabilitado puerto 1ffd
...
1111000000000011b=61443
	...
	*/
	if (MACHINE_IS_SPECTRUM_P2A_P3)
	{
		...
		if ((puerto_32765 & 32) == 32)
			return 255;

		if ((puerto & 61443) == 1)
			return (idle_bus_port_atribute() | 1);
	}
...
z80_byte teclado_matrix_que_filas(z80_byte fila, z80_byte puerto_h)
{
	...
	{
		if (i != fila)
		{ // No comparar fila con ella misma
			int bit;
			int mascara_bit = 1;
			for (bit = 0; bit < 5; bit++)
			{
...
z80_byte teclado_matrix_puerto_final(z80_byte puerto_h)
{
	...
	if (MACHINE_IS_SPECTRUM && keyboard_matrix_error.v)
	{
		...
		for (i = 0; i < 8; i++)
		{
			if ((puerto_h & mascara) == 0)
				final_puerto_h = teclado_matrix_que_filas(i, puerto_h);
			mascara = mascara << 1;
		}
...
z80_byte lee_puerto_teclado(z80_byte puerto_h)
{
	...
	if (initial_tap_load.v == 1 && initial_tap_sequence)
	{
		...
		{
			if (MACHINE_IS_ZX80_TYPE)
				return envia_load_pp_zx80(puerto_h);
			else
				return envia_load_pp_zx81(puerto_h);
		}
	}
...
//  other bits = Must be "1"    (the programs use compare FDh to test if bit1=0)
	...
	*/
	...
	if (MACHINE_IS_TBBLUE)
	{
		...
		if (puerto == TBBLUE_REGISTER_PORT)
			return tbblue_get_register_port();
		if (puerto == TBBLUE_VALUE_PORT)
			return tbblue_get_value_port();
		if (puerto == TBBLUE_SPRITE_INDEX_PORT)
			return tbblue_get_port_sprite_index();
		if (puerto == TBBLUE_LAYER2_PORT)
			return tbblue_get_port_layer2_value();

		if (puerto == DS1307_PORT_CLOCK)
			return ds1307_get_port_clock();
		if (puerto == DS1307_PORT_DATA)
			return ds1307_get_port_data();
		...
		if (puerto == TBBLUE_UART_RX_PORT)
			return tbblue_uartbridge_readdata();

		// puerto estado es el de escritura pero en lectura
		if (puerto == TBBLUE_UART_TX_PORT)
			return tbblue_uartbridge_readstatus();
	...
	// Puerto Spectra
	if (spectra_enabled.v && puerto == 0x7FDF)
		return spectra_read();

	// Sprite Chip
	if (spritechip_enabled.v && (puerto == SPRITECHIP_COMMAND_PORT || puerto == SPRITECHIP_DATA_PORT))
		return spritechip_read(puerto);
	...
	if (MACHINE_IS_PRISM)
	{
		if (puerto == 0x2ffd || puerto == 0x3ffd)
			return 255;
	...
	if (hardware_debug_port.v)
	{
		if (puerto == ZESARUX_ZXI_PORT_REGISTER)
			return zesarux_zxi_read_last_register();
		if (puerto == ZESARUX_ZXI_PORT_DATA)
			return zesarux_zxi_read_register_value();
	}
...
*/
	...
	{
		if (puerto == 32765 && si_chrome_features_enabled())
			return puerto_32765;
		if (puerto == 8189)
			return puerto_8189;
	}
	...
	{
		if (puerto_l == 0xBB)
			return gs_read_port_bb_from_speccy();
		if (puerto_l == 0xB3)
			return gs_read_port_b3_from_speccy();
	}
	...
	if (MACHINE_IS_TSCONF)
	{
		...
		if (puerto == 0xeff7)
			return puerto_eff7;
		if (puerto == 0xdff7)
			return zxevo_last_port_dff7;
		if (puerto == 0xbff7)
			return zxevo_nvram[zxevo_last_port_dff7];
		if (puerto_l == 0xaf)
			return tsconf_get_af_port(puerto_h);

		// Puertos ZIFI
		if (puerto == TSCONF_ZIFI_ERROR_REG)
			return tsconf_zifi_read_error_reg();
		if (puerto == TSCONF_ZIFI_DATA_REG)
			return tsconf_zifi_read_data_reg();
		if (puerto == TSCONF_ZIFI_INPUT_FIFO_STATUS)
			return tsconf_zifi_read_input_fifo_status();
		if (puerto == TSCONF_ZIFI_OUTPUT_FIFO_STATUS)
			return tsconf_zifi_read_output_fifo_status();

		// Puerto desconocido pero que usa la demo zifi. Tambien lo usa en escritura pero no se como va
		if (puerto == 0x57)
			return tsconf_read_port_57();
	...
	if (MACHINE_IS_BASECONF)
	{
		...
		if (puerto == 0xeff7 /*&& !baseconf_shadow_ports_available()*/)
			return puerto_eff7;

		if (puerto == 0xdff7 /*&& !baseconf_shadow_ports_available()*/)
			return zxevo_last_port_dff7;
		if (puerto == 0xdef7 /*&& baseconf_shadow_ports_available()*/)
			return zxevo_last_port_dff7;
...
void out_port_zx80_no_time(z80_int puerto, z80_byte value)
{
	...
	{
		if (puerto == ZESARUX_ZXI_ZX8081_PORT_REGISTER)
			zesarux_zxi_write_last_register(value);
		if (puerto == ZESARUX_ZXI_ZX8081_PORT_DATA)
			zesarux_zxi_write_register_value(value);
	}
	...
	if (rainbow_enabled.v)
	{
		...
		if (i >= 0 && i < CURRENT_FULLBORDER_ARRAY_LENGTH)
		{
			...
			if (MACHINE_IS_INVES)
			{
				...
				if (i == CURRENT_FULLBORDER_ARRAY_LENGTH)
					i--;
...
void out_port_spectrum_no_time(z80_int puerto, z80_byte value)
{
	...
	*/
	...
	if (((puerto & 49154) == 49152) || ((puerto & 49154) == 32768))
	{
		...
		if ((puerto & 49154) == 49152)
			puerto_final = 65533;
		else if ((puerto & 49154) == 32768)
			puerto_final = 49149;
	...
	if (MACHINE_IS_PENTAGON)
	{
		if (puerto == 0xeff7)
		{
			...
			if (pentagon_16c_mode_available.v)
			{
				...
				{
					if (value)
						screen_print_splash_text_center(ESTILO_GUI_TINTA_NORMAL, ESTILO_GUI_PAPEL_NORMAL, "Enabling 16C mode");
					else
	...
	{
		if (puerto_l == 0xBB)
			gs_write_port_bb_from_speccy(value);
		if (puerto_l == 0xB3)
			gs_write_port_b3_from_speccy(value);
	}
	...
	if (MACHINE_IS_TSCONF)
	{
		...
		if (puerto == 0xeff7)
			puerto_eff7 = value;
		if (puerto == 0xdff7)
			zxevo_last_port_dff7 = value;

		if (puerto == 0xbff7)
		{
			// Si esta permitida la escritura
			if (puerto_eff7 & 128)
				zxevo_nvram[zxevo_last_port_dff7] = value;
		}

		if (puerto_l == 0xaf)
			tsconf_write_af_port(puerto_h, value);
		...
		// Puertos ZIFI
		if (puerto == TSCONF_ZIFI_COMMAND_REG)
			tsconf_zifi_write_command_reg(value);
		if (puerto == TSCONF_ZIFI_DATA_REG)
			tsconf_zifi_write_data_reg(value);
	...
	if (MACHINE_IS_BASECONF)
	{
		if (puerto_l == 0xBF || puerto_l == 0x77 || (puerto & 0x0FFF) == 0xff7 || (puerto & 0x0FFF) == 0x7f7 || puerto == 0x7ffd || puerto == 0xeff7 || puerto == 0xEFF7 || puerto == 0xDFF7 || puerto == 0xDEF7 || puerto == 0xBFF7 || puerto == 0xBEF7)
		{
	...
	if (MACHINE_IS_TBBLUE)
	{
		...
		if (puerto == TBBLUE_REGISTER_PORT)
			tbblue_set_register_port(value);
		if (puerto == TBBLUE_VALUE_PORT)
		...
		if (puerto == TBBLUE_LAYER2_PORT)
			tbblue_out_port_layer2_value(value);

		// if (puerto_l==TBBLUE_SPRITE_PALETTE_PORT)	tbblue_out_sprite_palette(value);
		if (puerto_l == TBBLUE_SPRITE_PATTERN_PORT)
			tbblue_out_sprite_pattern(value);

		// Mantenemos puerto 0x55 port compatibilidad temporalmente. Se eliminara al subir nueva beta o la estable
		if (puerto_l == 0x55)
			tbblue_out_sprite_pattern(value);

		if (puerto_l == TBBLUE_SPRITE_SPRITE_PORT)
			tbblue_out_sprite_sprite(value);

		if (puerto == DS1307_PORT_CLOCK)
			ds1307_write_port_clock(value);
		if (puerto == DS1307_PORT_DATA)
			ds1307_write_port_data(value);

		if (puerto == TBBLUE_UART_TX_PORT)
			tbblue_uartbridge_writedata(value);
	}
	...
	if (audiodac_enabled.v && puerto_l == audiodac_types[audiodac_selected_type].port)
	{
		...
		if (puerto_l == 0xDF && reg_pc < 0x2000)
			sonido = 0;
